...
export function checkRules(doc) {
    ...
    if (h1min === undefined)
        warnings.push({ code: 'H1_CAP_MISSING', msg: 'caps.max_outflow_h1 not set (no global rate limit)', path: 'caps.max_outflow_h1' });
    if (d1min === undefined)
        warnings.push({ code: 'D1_CAP_MISSING', msg: 'caps.max_outflow_d1 not set (no daily limit)', path: 'caps.max_outflow_d1' });
