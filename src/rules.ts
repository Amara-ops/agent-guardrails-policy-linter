...
export function checkRules(doc: any): { errors: Finding[]; warnings: Finding[]; suggestions: Finding[] } {
  ...
  if (Array.isArray(doc?.calls?.denylist)) {
    ...
    for (let i = 0; i < dl.length; i++) {
      const t = dl[i];
      const bad = !Array.isArray(t) || t.length !== 3 || typeof t[0] !== 'number' || !/^0x[0-9a-fA-F]{40}$/.test(t[1]) || !/^0x[0-9a-fA-F]{8}$/.test(t[2]);
      if (bad) warnings.push({ code: 'DENY_TRIPLE_INVALID', msg: `denylist[${i}] should be [chainId, addr, selector]`, path: `calls.denylist[${i}]` });
      // conflict check
      if (Array.isArray(al)) {
        const conflict = al.some(([c,a,s]: any) => c===t[0] && a===t[1] && s===t[2]);
        if (conflict) warnings.push({ code: 'DENYLIST_CONFLICT', msg: `denylist entry also present in allowlist`, path: `calls.denylist[${i}]` });
      }
    }
  }
  ...
  const hasSwap = Array.isArray(al) && al.some(([, , s]: any) => s?.toLowerCase() === '0x04e45aaf');
  if (hasSwap) {
